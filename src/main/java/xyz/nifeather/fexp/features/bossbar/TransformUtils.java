...
public class TransformUtils
{
    ...
    private static class ValueTransformer
    {
        ...
        public static BossbarColor valueAt(double progress, BossbarColor startVal, BossbarColor endVal, Easing easing)
        {
            if (startVal.equals(endVal)) return endVal;
            if (progress <= 0) return startVal;
            if (progress >= 1) return endVal;

            var r = valueAt(progress, startVal.red(), endVal.red(), easing);
            var g = valueAt(progress, startVal.green(), endVal.green(), easing);
            var b = valueAt(progress, startVal.blue(), endVal.blue(), easing);

            return new BossbarColor(r, g, b);
        }
